...
const MapContainer = React.forwardRef<MapContainerRef, MapContainerProps>(
  ({ preferences, onPreferenceToggle, onMapClick, onLocationReset }, ref) => {
    // 선택된 시설 및 클러스터 상태
    ...
    // 무더위 쉼터 상태 (위치 기반)
    const {
      facilities: coolingShelterFacilities,
      error: coolingShelterError,
    ...
    const {
      ...
      error: culturalReservationsError,
    ...
    const facilitiesFromPOIs = useMemo(
      () =>
        pois
          ...
          .filter(facility => facility !== null) as Facility[],
      [pois]
    );
    ...
    const facilitiesFromLibraries = useMemo(
      () =>
        libraries.map(library => ({
          ...
          description: `${library.lbrrySeName || ''} | ${library.codeValue || ''}`
            .trim()
            .replace(/^\|\s*|\s*\|$/g, ''),
          congestionLevel: 'low' as const,
    ...
    const facilitiesFromCulturalEvents = useMemo(
      () =>
        culturalEvents.map(event => ({
          ...
          description: `${event.codeName || ''} | ${event.useTarget || ''}`
            .trim()
            .replace(/^\|\s*|\s*\|$/g, ''),
          congestionLevel: 'low' as const,
    ...
    const allFacilities = useMemo(
      () => [
        ...facilitiesFromPOIs,
        ...facilitiesFromBikes,
        ...coolingShelterFacilities,
        ...facilitiesFromParks,
        ...facilitiesFromLibraries,
        ...facilitiesFromCulturalSpaces,
        ...facilitiesFromCulturalEvents,
        ...facilitiesFromCulturalReservations,
        ...facilitiesFromRestaurants,
      ],
      [
        facilitiesFromPOIs,
        facilitiesFromBikes,
    ...
    const handleLocationChange = useCallback(
      async (lat: number, lng: number) => {
        ...
        try {
          ...
          const promises = [
            ...
            // 무더위 쉼터 위치 기반 호출
            fetchCoolingShelters(lat, lng).then(() =>
              console.log('무더위 쉼터 데이터:', coolingShelterFacilities.length, '개')
            ),
            // 도서관 위치 기반 호출
            fetchLibraries(lat, lng).then(() =>
              console.log('도서관 데이터:', libraries.length, '개')
            ),
            // 문화공간 위치 기반 호출
            fetchCulturalSpaces(lat, lng).then(() =>
              console.log('문화공간 데이터:', culturalSpaces.length, '개')
            ),
            // 문화행사 위치 기반 호출
            fetchCulturalEvents(lat, lng).then(() =>
              console.log('문화행사 데이터:', culturalEvents.length, '개')
            ),
            // 문화예약 위치 기반 호출
            fetchCulturalReservations(lat, lng).then(() =>
              console.log('문화예약 데이터:', culturalReservations.length, '개')
            ),
            // 맛집 위치 기반 호출
            fetchRestaurants(lat, lng).then(() =>
              console.log('맛집 데이터:', restaurants.length, '개')
            ),
          ];
      ...
      [
        ...
        fetchRestaurants,
        parks.length,
        coolingShelterFacilities.length,
        libraries.length,
        culturalSpaces.length,
        culturalEvents.length,
        culturalReservations.length,
        restaurants.length,
      ]
    );
    ...
    const handleMapMove = useCallback(
      (lat: number, lng: number, retryCount: number = 0) => {
        ...
        if (
          ...
          !mapStatus.success ||
          mapStatus.loading ||
          !(window as KakaoWindow).kakao?.maps
        ) {
    ...
    const handleSearchSelect = useCallback(
      async (searchItem: SearchItem) => {
        await selectSearchResult(searchItem, handleMapMove, setSelectedFacility);
      },
