...
const ClusterBottomSheet = React.memo(function ClusterBottomSheet({ 
  ...
  const groupedByCategory = useMemo(() => {
    ...
    return cluster.facilities.reduce((acc, facility) => {
                ...
                return (
                  <div key={category} className="space-y-2">
                    <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                      ...
                      >
                        <div className="w-4 h-4 text-white" dangerouslySetInnerHTML={{ __html: icon.svg }} />
                      </div>
            ...
            <div className="p-4 space-y-4 overflow-y-auto max-h-[calc(80vh-120px)]">
              {selectedFacility && (
                <>
                  ...
                  <div className="space-y-3">
                    {selectedFacility.address && (
                      <div className="flex items-start space-x-3">
                        ...
                        <p className="text-gray-700 text-sm leading-relaxed">{selectedFacility.address}</p>
                      </div>
                    )}
                    ...
                    {selectedFacility.operatingHours && (
                      <div className="flex items-start space-x-3">
                        ...
                        <p className="text-gray-700 text-sm leading-relaxed">{selectedFacility.operatingHours}</p>
                      </div>
                    )}
                  ...
                  {/* 문화행사 전용 정보 */}
                  {selectedFacility.category === 'cultural_event' && selectedFacility.culturalEvent && (
                      ...
                      <div className="space-y-3 text-sm">
                        {selectedFacility.culturalEvent.codeName && (
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-900 font-medium">{selectedFacility.culturalEvent.codeName}</span>
                          </div>
                        )}
                        {selectedFacility.culturalEvent.eventDate && (
                          <div className="flex justify-between">
                            <span className="text-gray-500">기간:</span>
                            <span className="text-gray-900 font-medium">{selectedFacility.culturalEvent.eventDate}</span>
                          </div>
                        )}
                        {selectedFacility.culturalEvent.useFee && (
                          <div className="flex flex-col space-y-1">
                            <span className="text-gray-500">요금:</span>
                            <span className="text-gray-900 text-xs leading-relaxed">{selectedFacility.culturalEvent.useFee}</span>
                          </div>
                        )}
                        {selectedFacility.culturalEvent.useTarget && (
                          <div className="flex flex-col space-y-1">
                            <span className="text-gray-500">이용대상:</span>
                            <span className="text-gray-900 text-xs">{selectedFacility.culturalEvent.useTarget}</span>
                          </div>
                        )}
                        {selectedFacility.culturalEvent.isFree && (
                          <div className="flex justify-between">
                            ...
                            <span className={`font-medium ${
                              selectedFacility.culturalEvent.isFree === '무료' ? 'text-green-600' : 'text-blue-600'
                            }`}>
                              {selectedFacility.culturalEvent.isFree}
                            </span>
                          </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      ...
                      {selectedFacility.distance && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-gray-900 font-medium">{selectedFacility.distance.toFixed(1)}km</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        <span className="text-gray-500">예약:</span>
                        <span className="text-gray-900 font-medium">{selectedFacility.isReservable ? '가능' : '불가'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedFacility.description && (
                    <div className="border-t pt-4">
                      ...
                      <p className="text-gray-700 text-sm leading-relaxed">{selectedFacility.description}</p>
                    </div>
                  )}
                </>
              )}
