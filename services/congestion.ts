import {CongestionData, SeoulApiResponse} from '@/lib/types';
 ...
 * (서울 열린 데이터 광장에서 제공한 서울시 주요 120 장소 목록 참고)
 * @param lat 현재 위치 위도
 * @param lng 현재 위치 경도
 * @return 가장 가까운 장소의 혼잡도 정보
 */
export async function getNearestCongestionData(lat: number, lng: number): Promise<CongestionData | null> {
    try {
        // 현재 위치에서 가장 가까운 장소 찾기
        const nearestAreaCode = findNearestAreaCode(lat, lng);

        // 서울시 실시간 인구데이터 API 호출
        const url = `${BASE_URL}/${API_KEY}/json/citydata_ppltn/1/5/${nearestAreaCode}`;
        ...
        if (data.RESULT?.['RESULT.CODE'] !== 'INFO-000') {
            console.error(`API 에러: ${data.RESULT?.['RESULT.MESSAGE']}`);
            return null;
        }
        ...
        if (!congestionArray || congestionArray.length === 0) {
            console.warn(`${nearestAreaCode}에 대한 혼잡도 데이터가 없습니다.`);
            return null;
        }

        return congestionArray[0];
    } catch (error) {
