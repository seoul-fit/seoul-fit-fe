...
export function useSearchDetail() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data?.stations) {
            ...
            const station = data.data.stations.find((s: SubwayStationDetail) => 
              s.code === searchItem.id || s.name === searchItem.name
            );
            if (station) {
              return {
                id: searchItem.id,
                ...
                category: 'subway',
                position: { lat: station.lat || 37.5665, lng: station.lng || 126.9780 },
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data?.stations) {
            ...
            const station = data.data.stations.find((s: BikeStationDetail) => 
              s.code === searchItem.id || s.name === searchItem.name
            );
            if (station) {
              return {
                ...
                bikeFacility: {
                  ...
                  availableRacks: parseInt(station.rackTotCnt) - parseInt(station.parkingBikeTotCnt) || 0,
  ...
  // 상세 데이터 조회
  const fetchSearchDetail = useCallback(async (searchItem: SearchItem): Promise<Facility | null> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '상세 데이터를 불러오는데 실패했습니다.';
