...
const DEFAULT_PREFERENCES: UserPreferences = {
  ...
  [FACILITY_CATEGORIES.CULTURAL_EVENT]: true,
  [FACILITY_CATEGORIES.CULTURAL_RESERVATION]: true,
};
...
export const useFacilities = (options: UseFacilitiesOptions = {}) => {
  const { initialPreferences = DEFAULT_PREFERENCES, facilities: externalFacilities } = options;
  ...
  const visibleFacilities = useMemo(
    () => facilities.filter(facility => preferences[facility.category]),
  ...
  const searchFacilities = useCallback(
    (query: string): Facility[] => {
      if (!query.trim()) return visibleFacilities;

      const lowerQuery = query.toLowerCase();
      return visibleFacilities.filter(
        facility =>
  ...
  const facilitiesByCongest = useMemo(() => {
    ...
    return [...visibleFacilities].sort(
      (a, b) => congestionOrder[a.congestionLevel] - congestionOrder[b.congestionLevel]
    );
  }, [visibleFacilities]);
  ...
  const getFacilitiesByCategory = useCallback(
    (category: FacilityCategory): Facility[] => {
  ...
  // 활성화된 카테고리 개수
  const enabledCategoryCount = useMemo(() => enabledCategories.length, [enabledCategories]);
