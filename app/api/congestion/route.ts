...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!response.ok) {
            console.error(`서울시 실시간 인구데이터 API 호출 실패: ${response.status} ${response.statusText}`);
            return NextResponse.json(
                {
                    error: `서울시 실시간 인구데이터 API 호출 실패: ${response.status}`,
        ...
        if (data.RESULT?.['RESULT.CODE'] !== 'INFO-000') {
            ...
            return NextResponse.json(
                {
                    error: `서울시 실시간 인구데이터 API 응답 에러: ${data.RESULT?.['RESULT.MESSAGE']}`,
                    code: 'API_ERROR'
                },
                { status: 502 }
            );
